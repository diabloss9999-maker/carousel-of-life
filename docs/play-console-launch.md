...
## Step 1 — Play Console 계정 (이미 본인인증 완료)

https://play.google.com/console 접속.
조직(기업) 계정으로 가입 — 사업자 정보:
...
| 연락처 전화번호 | 010-9426-9495 |
| D-U-N-S 번호 (필요시) | (요청 시 등록) |

**1회 등록비**: $25 (개인) 또는 사업자 명의(권장).
...
### 4-1. 앱 세부정보
- **앱 이름**: 인생의 회전목마
- **간단한 설명** (80자): 별의 흐름과 카드의 계시로 오늘의 운명을 읽어드리는 AI 운세 앱.
...
- **휴대전화 스크린샷** (최소 2장, 16:9 또는 9:16): `android/screenshots/01-landing.png`, `02-pricing.png`
  - ⚠️ **추가 권장** — 더 많은 화면 (운세 결과·점술사 채팅·이름 궁합 등 6~8장)
- **태블릿 스크린샷** (선택): 추가 권장
...
### 5-1. 결제 정책 신고
앱 설정 → **결제 정책 신고**:
- ✅ **외부 결제 시스템(NHN KCP) 사용** 신고
- 신고 사유: "한국 사용자 대상 외부 결제 시스템 사용 (전자상거래법 준수)"
- 사용자에게 외부 결제 사용 안내 화면 노출 — **이미 구현됨** (`ExternalPaymentNotice` 컴포넌트, `/pricing` 페이지)

### 5-2. 안내 문구 확인
앱 내부에 다음 문구 노출 (이미 구현):
> "본 앱의 결제는 Google Play 결제가 아닌 외부 결제 시스템(NHN KCP)을 통해 진행됩니다."
