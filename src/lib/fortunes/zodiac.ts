...
export function getZodiacSign(birthDate: string): ZodiacInfo {
  ...
  for (const zodiac of ZODIAC_LIST) {
    ...
    if (startMonth <= endMonth) {
      if (
        (mm === startMonth && dd >= startDay) ||
        (mm === endMonth && dd <= endDay) ||
        (mm > startMonth && mm < endMonth)
      ) {
        return zodiac;
      }
    } else if (
...
export function getChineseZodiac(birthDate: string): ChineseZodiacInfo {
  ...
  return CHINESE_ZODIAC_LIST[(index + 12) % 12];
}
