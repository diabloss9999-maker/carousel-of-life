...
function buildDetail(me: TypeInfo, partner: TypeInfo): string {
  const matches = axisMatchCount(me.type, partner.type);

  if (me.compatibleWith.includes(partner.type)) {
    return `${me.nickname}과 ${partner.nickname}은 서로의 부족한 부분을 자연스럽게 채워주는 짝이야. ${me.type}의 ${me.strengths[0]}과 ${partner.type}의 ${partner.strengths[0]}이 만나면, 함께 있는 시간이 길수록 더 단단해져.`;
  }
  if (me.incompatibleWith.includes(partner.type)) {
    return `${me.nickname}과 ${partner.nickname}은 가치관과 우선순위가 달라 처음엔 부딪힐 수 있어. 하지만 차이를 인정하고 솔직하게 대화하면, 서로 가장 큰 자극이 되는 관계로 발전할 수 있어.`;
  }
  if (matches >= 3) {
    return `네 축 중 ${matches}개가 같아 일상의 리듬과 결정 방식이 비슷해. 큰 갈등은 적지만 비슷한 단점도 공유하니, 한 명이라도 다른 시선을 가져주면 균형이 잡혀.`;
  }
  if (matches === 2) {
    return `반은 닮고 반은 다른 사이라, 서로에게 신선한 자극을 주면서도 안정감을 느낄 수 있어. ${me.strengths[0]}과 ${partner.strengths[0]}을 잘 살리면 좋은 팀이 돼.`;
  }
  return `${me.nickname}과 ${partner.nickname}은 사고와 행동 방식이 거의 정반대야. 처음엔 답답할 수 있지만, 그만큼 서로에게 새로운 세상을 보여줄 수 있어. 인내심이 관계의 열쇠야.`;
}
