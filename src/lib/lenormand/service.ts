...
[해석 원칙]
- 중심 카드([4])가 핵심 답. 가장 비중 있게 다뤄.
- 인접 카드 조합으로 이야기 만들기 (예: 구름+열쇠 = "혼란이 해결된다").
- 상단=과거, 중단=현재, 하단=미래의 흐름을 자연스럽게 연결.
- 부정 카드도 솔직히 짚되 대응 방향을 함께 제시.
- 600~900자, 뽑힌 카드명을 반드시 인용.
- 점쟁이 흉내 X, 예언 투·신비주의 표현 금지.

JSON 금지. 아래 마크다운 헤딩으로 구조화해서 응답:

## 🎯 핵심 메시지
(중심 카드 중심의 핵심 답, 2~3문장)

## 📜 카드 배열 해석
(상단·중단·하단 흐름, 4~6문장)

## 🔍 주목할 조합
(눈에 띄는 카드 조합 2~3개, 각 1문장)
...
[해석 원칙]
- 시그니피케이터(${sigLine})를 중심으로 해석한다.
- 8방향 인접 카드의 영향력을 구체적으로 풀어준다.
- "왼쪽=과거 / 오른쪽=미래" 원칙으로 같은 행을 시간선처럼 읽는다.
- 같은 열은 그 영역의 인생 기둥(반복되는 테마).
- 영혼 카드 4장은 내면 깊은 흐름.
- 부정 카드도 솔직히 짚되 대응 방향을 함께 제시.
- 점쟁이 흉내 X, 예언 투·신비주의 표현 금지.
- 900~1500자, 뽑힌 카드명을 반드시 인용.

JSON 금지. 아래 마크다운 헤딩으로 구조화해서 응답:

## 🪞 시그니피케이터의 자리
(시그니피케이터 위치·주변 분위기 요약, 2~3문장)

## ⏳ 과거 → 미래의 흐름
(같은 행 왼쪽=과거, 오른쪽=미래 시간선 해석, 4~6문장)

## 🌐 8방향이 말하는 영향
(위/아래/대각선 카드 영향, 3~5문장)

## 🏛️ 인생의 기둥 (같은 열)
(시그니피케이터의 같은 열 4장 해석, 2~4문장)
