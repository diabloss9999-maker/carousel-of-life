...
/** 감지 패턴 */
const PATTERNS: { type: ReadingType; regex: RegExp }[] = [
  { type: "tarot3",     regex: /타로.*(3장|세\s*장|3|세장|과거.현재|과거.*미래)/i },
...
export function detectAndDraw(message: string): ReadingResult | null {
  for (const { type, regex } of PATTERNS) {
    ...
    if (type === "tarot3") {
      ...
      const lines = cards.map(c =>
        `${c.position}: ${c.nameKo} — ${c.isReversed ? "역방향" : "정방향"}`
      ).join("\n");
      return {
        type,
        cards,
        promptText: `[지금 막 타로 3장이 뽑혔어 — 과거·현재·미래]\n${lines}\n` +
    ...
    if (type === "rune1") {
      ...
      return {
        ...
        cards: [{
          ...
          meaning: r.isReversed ? (r.rune.meaningReversed ?? r.rune.meaningUpright) : r.rune.meaningUpright,
        }],
        promptText: `[지금 막 룬이 뽑혔어]\n` +
