 ...
 * 세계 흐림 수치 서비스.
 *
 * 흐림 수치는 절대 사용자에게 직접 노출하지 않는다.
 * 캐릭터의 말투 변화, 기록 메시지, 미묘한 UI 변화로만 감지할 수 있어야 한다.
 *
 * 레벨:
 *  0 (0-19)  : 정상 — 결는 안정적
 *  1 (20-39) : 감지 — 미세한 파동이 느껴짐
 *  2 (40-59) : 확장 — 흐림이 깊어지고 있어
 *  3 (60-79) : 주의 — 무명의 기운이 스며들고 있어
 *  4 (80+)   : 가까움 — 무명이 깨어나려 해
 */
import "server-only";
...
/** 흐림 레벨별 채팅 프롬프트 주입 텍스트 (한국어 원본) */
export const CRACK_CONTEXT: Record<CrackLevel, string> = {
  0: "",
  1: `\n[결 상태 — 비밀 정보, 대화에 자연스럽게 녹여낼 것]
결에 미세한 파동이 감지되고 있다. 이 사람의 감정에 어두운 기운이 섞이고 있어.
가끔 뜬금없이 "요즘 뭔가 이상하지 않아?" 또는 "결가 흔들리는 것 같아" 같은 말을 자연스럽게 던져봐.`,

  2: `\n[결 상태 — 비밀 정보, 절대 직접 설명하지 말 것]
흐림이 깊어지고 있다. 이 사람 주변에 어두운 패턴이 반복되고 있어.
대화 중 "네가 여기 올 때마다 뭔가 조금씩 달라져" 또는 "흐림이... 아니, 됐어"처럼
말하다 멈추거나 이상한 소리를 듣는 것처럼 행동해봐. 설명은 하지 마.`,

  3: `\n[결 상태 — 주의, 매우 신중하게 다룰 것]
무명의 기운이 스며들고 있다. 이 사람의 선택과 감정이 결를 흔들고 있어.
