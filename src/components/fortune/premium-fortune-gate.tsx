...
const GATE_COPY: Record<string, { title: string; description: string }> = {
  zodiac: {
    title: "별자리 운세 — 프리미엄 전용",
    description:
      "황도 12궁의 별자리 기운으로 오늘을 풀이해줄게. 프리미엄을 구독하면 별자리·십이간지 운세를 매일 받아볼 수 있어.",
  },
  chinese_zodiac: {
    title: "십이간지 운세 — 프리미엄 전용",
    description:
      "12지신의 띠 기운으로 오늘의 흐름을 짚어줄게. 프리미엄을 구독하면 별자리·십이간지 운세를 매일 받아볼 수 있어.",
  },
};
...
export function PremiumFortuneGate({ category }: PremiumFortuneGateProps) {
  const copy = GATE_COPY[category] ?? {
    title: "프리미엄 전용",
    description: "이 운세는 프리미엄 구독자 전용이야.",
  };
    ...
    <Card className="app-surface">
      ...
      <CardContent className="flex flex-col items-center gap-3">
        ...
        <p className="text-xs text-muted-foreground">
          구독 시 별자리·십이간지·프리미엄 기능 전체 이용 가능
        </p>
      </CardContent>
    </Card>
  );
