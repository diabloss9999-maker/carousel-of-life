...
export function FateLogView({ entries, summary, crackLevel }: FateLogViewProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <p className="text-[15px] uppercase tracking-widest text-muted-foreground/70">{t("myStory")}</p>
        ...
        {summary.narrative.repeatedCard && (
          <div className="rounded-xl border border-white/5 bg-white/3 px-4 py-3">
            <p className="text-[15px] text-muted-foreground/70 tracking-widest mb-1">{t("repeatedDetected")}</p>
            <p className="text-[15px] text-muted-foreground/70 font-mystic italic">
              {t("repeatedCardLine", {
        ...
        {summary.narrative.characterCounts.length > 0 && (
          <div className="space-y-2 border-t border-white/5 pt-4">
            <p className="text-[15px] text-muted-foreground/70 tracking-widest">{t("encounter")}</p>
            <div className="flex flex-wrap gap-2">
              {summary.narrative.characterCounts.map(({ name, count: cnt }) => (
                <span key={name} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[15px] text-muted-foreground/85">
        ...
        {summary.patterns.length > 0 && (
          <div className="border-t border-white/5 pt-3 space-y-1">
            <p className="text-[15px] text-muted-foreground/70 tracking-widest">{t("patternsDetected")}</p>
            {summary.patterns.map((p, i) => (
              <p key={i} className="text-[15px] text-muted-foreground/70 font-mystic italic">{p}</p>
            ))}
          </div>
        )}
        ...
        <div className="border-t border-white/5 pt-3 flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[15px] text-muted-foreground/75 hover:text-muted-foreground/95 transition-colors"
          >
      ...
      {entries.length === 0 ? (
        <div className="py-16 text-center space-y-2">
          ...
          <p className="text-[15px] text-muted-foreground/65">{t("emptyBody")}</p>
        </div>
      ) : (
              ...
              <div key={entry.id} className="relative flex gap-4 py-2">
                ...
                >
                  <div className="flex items-baseline justify-between gap-2">
                    <p className={cn(
                      "font-mystic text-[15px] leading-snug transition-colors",
                      isOpen ? "text-foreground" : "text-foreground/70 group-hover:text-foreground/90",
                    )}>
                      {entry.title}
                    </p>
                    <span className="text-[15px] text-muted-foreground/65 flex-shrink-0 tabular-nums">
                      {formatDate(entry.date)}
                    </span>
                  </div>
                  {entry.detail && (
                    <p className="text-[15px] text-muted-foreground/70">{entry.detail}</p>
                  )}
                  {isOpen && (
                    <p className="text-[15px] text-muted-foreground/60 font-mystic italic pt-1 border-t border-white/5 mt-1">
                      {entry.significance}
                    </p>
                  )}
                </button>
...
function StatCell({
  ...
  return (
    <div className="space-y-1">
      <p className="text-[15px] uppercase tracking-widest text-muted-foreground/70">{label}</p>
      <p className={cn(
        "font-mystic text-[15px] font-semibold",
